...
const next_i18next_config_1 = require("../../../../config/next-i18next.config");
const activity_1 = require("../../../interfaces/activity");
const logger_1 = tslib_1.__importDefault(require("../../../utils/logger"));
const add_activity_1 = require("../../middlewares/add-activity");
const apiv3_form_validator_1 = require("../../middlewares/apiv3-form-validator");
const core_1 = require("@growi/core");
const logger = (0, logger_1.default)('growi:routes:apiv3:app-settings');
const { pathUtils } = require('@growi/core');
const debug = require('../../../utils/logger/alias-for-debug')('growi:routes:admin');
...
module.exports = (crowi) => {
    ...
    const validator = {
        appSetting: [
            ...
            (0, express_validator_1.body)('confidential'),
            (0, express_validator_1.body)('globalLang').isIn(next_i18next_config_1.i18n.locales),
            (0, express_validator_1.body)('isEmailPublishedForNewUser').isBoolean(),
            (0, express_validator_1.body)('fileUpload').isBoolean(),
        ],
        siteUrlSetting: [
            // https://regex101.com/r/5Xef8V/1
            (0, express_validator_1.body)('siteUrl').trim().matches(/^(https?:\/\/)/).isURL({ require_tld: false }),
        ],
        mailSetting: [
            (0, express_validator_1.body)('fromAddress').trim().if(value => value !== '').isEmail(),
            (0, express_validator_1.body)('transmissionMethod').isIn(['smtp', 'ses']),
        ],
        smtpSetting: [
            (0, express_validator_1.body)('smtpHost').trim(),
            (0, express_validator_1.body)('smtpPort').trim().if(value => value !== '').isPort(),
            (0, express_validator_1.body)('smtpUser').trim(),
            (0, express_validator_1.body)('smtpPassword').trim(),
        ],
        sesSetting: [
            (0, express_validator_1.body)('sesAccessKeyId').trim().if(value => value !== '').matches(/^[\da-zA-Z]+$/),
            (0, express_validator_1.body)('sesSecretAccessKey').trim(),
        ],
        fileUploadSetting: [
            (0, express_validator_1.body)('fileUploadType').isIn(['aws', 'gcs', 'local', 'gridfs']),
            (0, express_validator_1.body)('gcsApiKeyJsonPath').trim(),
            (0, express_validator_1.body)('gcsBucket').trim(),
            (0, express_validator_1.body)('gcsUploadNamespace').trim(),
            (0, express_validator_1.body)('gcsReferenceFileWithRelayMode').if(value => value != null).isBoolean(),
            (0, express_validator_1.body)('s3Region').trim().if(value => value !== '').matches(/^[a-z]+$/)
                .withMessage((value, { req }) => req.t('validation.aws_region')),
            (0, express_validator_1.body)('s3CustomEndpoint').trim().if(value => value !== '').matches(/^(https?:\/\/[^/]+|)$/)
                .withMessage((value, { req }) => req.t('validation.aws_custom_endpoint')),
            (0, express_validator_1.body)('s3Bucket').trim(),
            (0, express_validator_1.body)('s3AccessKeyId').trim().if(value => value !== '').matches(/^[\da-zA-Z]+$/),
            (0, express_validator_1.body)('s3SecretAccessKey').trim(),
            (0, express_validator_1.body)('s3ReferenceFileWithRelayMode').if(value => value != null).isBoolean(),
        ],
        maintenanceMode: [
            (0, express_validator_1.body)('flag').isBoolean(),
        ],
    };
    /**
     * @swagger
     ...
     *                  $ref: '#/components/schemas/AppSettingParams'
     */
    router.put('/app-setting', loginRequiredStrictly, adminRequired, addActivity, validator.appSetting, apiv3_form_validator_1.apiV3FormValidator, async (req, res) => {
        ...
        try {
            ...
            const parameters = { action: activity_1.SupportedAction.ACTION_ADMIN_APP_SETTINGS_UPDATE };
            ...
            return res.apiv3Err(new core_1.ErrorV3(msg, 'update-appSetting-failed'));
        }
    });
    /**
     * @swagger
     ...
     *                  $ref: '#/components/schemas/SiteUrlSettingParams'
     */
    router.put('/site-url-setting', loginRequiredStrictly, adminRequired, addActivity, validator.siteUrlSetting, apiv3_form_validator_1.apiV3FormValidator, async (req, res) => {
        ...
        try {
            ...
            const parameters = { action: activity_1.SupportedAction.ACTION_ADMIN_SITE_URL_UPDATE };
            activityEvent.emit('update', res.locals.activity._id, parameters);
            ...
            logger.error('Error', err);
            return res.apiv3Err(new core_1.ErrorV3(msg, 'update-siteUrlSetting-failed'));
        }
    });
    /**
     ...
     *                  $ref: '#/components/schemas/SmtpSettingParams'
     */
    router.put('/smtp-setting', loginRequiredStrictly, adminRequired, addActivity, validator.smtpSetting, apiv3_form_validator_1.apiV3FormValidator, async (req, res) => {
        ...
        try {
            ...
            const parameters = { action: activity_1.SupportedAction.ACTION_ADMIN_MAIL_SMTP_UPDATE };
            activityEvent.emit('update', res.locals.activity._id, parameters);
            ...
            logger.error('Error', err);
            return res.apiv3Err(new core_1.ErrorV3(msg, 'update-smtpSetting-failed'));
        }
    });
    /**
     * @swagger
    ...
    router.post('/smtp-test', loginRequiredStrictly, adminRequired, addActivity, async (req, res) => {
        try {
            ...
            const parameters = { action: activity_1.SupportedAction.ACTION_ADMIN_MAIL_TEST_SUBMIT };
            activityEvent.emit('update', res.locals.activity._id, parameters);
            return res.apiv3({});
        }
        catch (err) {
            ...
            return res.apiv3Err(new core_1.ErrorV3(msg, 'send-email-with-smtp-failed'));
        }
    });
    /**
     * @swagger
     ...
     *                  $ref: '#/components/schemas/SesSettingParams'
     */
    router.put('/ses-setting', loginRequiredStrictly, adminRequired, addActivity, validator.sesSetting, apiv3_form_validator_1.apiV3FormValidator, async (req, res) => {
        ...
        try {
            ...
            logger.error('Error', err);
            return res.apiv3Err(new core_1.ErrorV3(msg, 'update-ses-setting-failed'));
        }
        await mailService.initialize();
        mailService.publishUpdatedMessage();
        const parameters = { action: activity_1.SupportedAction.ACTION_ADMIN_MAIL_SES_UPDATE };
        activityEvent.emit('update', res.locals.activity._id, parameters);
        return res.apiv3({ mailSettingParams });
    });
    /**
    ...
    router.put('/file-upload-setting', loginRequiredStrictly, adminRequired, addActivity, validator.fileUploadSetting, apiv3_form_validator_1.apiV3FormValidator, async (req, res) => {
        ...
        try {
            ...
            const parameters = { action: activity_1.SupportedAction.ACTION_ADMIN_FILE_UPLOAD_CONFIG_UPDATE };
            activityEvent.emit('update', res.locals.activity._id, parameters);
            ...
            logger.error('Error', err);
            return res.apiv3Err(new core_1.ErrorV3(msg, 'update-fileUploadType-failed'));
        }
    });
    router.post('/v5-schema-migration', accessTokenParser, loginRequiredStrictly, adminRequired, async (req, res) => {
        const isMaintenanceMode = crowi.appService.isMaintenanceMode();
        if (!isMaintenanceMode) {
            return res.apiv3Err(new core_1.ErrorV3('GROWI is not maintenance mode. To import data, please activate the maintenance mode first.', 'not_maintenance_mode'));
        }
        ...
        try {
            ...
            return res.apiv3Err(new core_1.ErrorV3(`Failed to migrate pages: ${err.message}`), 500);
        }
        return res.apiv3({ isV5Compatible });
    });
    // eslint-disable-next-line max-len
    router.post('/maintenance-mode', accessTokenParser, loginRequiredStrictly, adminRequired, addActivity, validator.maintenanceMode, apiv3_form_validator_1.apiV3FormValidator, async (req, res) => {
        const { flag } = req.body;
        const parameters = {};
        try {
            if (flag) {
                await crowi.appService.startMaintenanceMode();
                Object.assign(parameters, { action: activity_1.SupportedAction.ACTION_ADMIN_MAINTENANCEMODE_ENABLED });
            }
            else {
                await crowi.appService.endMaintenanceMode();
                Object.assign(parameters, { action: activity_1.SupportedAction.ACTION_ADMIN_MAINTENANCEMODE_DISABLED });
            }
        }
        catch (err) {
            logger.error(err);
            if (flag) {
                res.apiv3Err(new core_1.ErrorV3('Failed to start maintenance mode', 'failed_to_start_maintenance_mode'), 500);
            }
            else {
                res.apiv3Err(new core_1.ErrorV3('Failed to end maintenance mode', 'failed_to_end_maintenance_mode'), 500);
            }
        }
